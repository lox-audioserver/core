...
export type TuneInUsernameCheck = { found: false } | { found: true; presets: TuneInPreset[] };

/**
 * Whether a TuneIn username resolves, and which playable presets it has.
 *
 ...
 * - Presets filed in folders or grouped into sections are stations too. Reading only the top
 *   level reported none for accounts that had plenty.
 *
...
export async function validateTuneInUsername(username: string): Promise<TuneInUsernameCheck> {
  ...
  return { found: true, presets: listPlayablePresets(expanded) };
}
